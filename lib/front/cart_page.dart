...
// ---------------- IF NO TIP → SHOW ADD BUTTON ----------------
                    if (cart.tipAmount == 0)
                      GestureDetector(
                        onTap: () {
                          final tip =
                              double.tryParse(_tipController.text.trim()) ??
                                  0.0;
                          if (tip > 0) {
                            cart.setTip(tip);
...
// ---------------- IF TIP ADDED → SHOW REMOVE BUTTON ----------------
              ...
              _AmountDetailsRow(label: "Tax ", value: cart.tax, bold: false),
              _AmountDetailsRow(
                  label: "Pick Up From Store", value: 0.0, bold: false),
              if (cart.tipAmount > 0)
                _AmountDetailsRow(
                    label: "Tip", value: cart.tipAmount, bold: false),
...
// --- Show Special Instruction (if exists) ---
                if (item.specialInstruction != null &&
                    item.specialInstruction!.isNotEmpty) ...[
  ...
  Widget _buildCartContent(BuildContext context,
    ...
    final double grandTotal = cart.total;
            ...
            Expanded(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildAmountRow("Tax:", tax, fontSize: fontSize),
                  const Divider(),
...
class VoucherInputField extends StatefulWidget {
  final Future<void> Function(String code)
      onApply; // <-- make it async-friendly
    ...
    builder: (_) => Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
              color: success ? Colors.green : Colors.red, width: 1.5),
          boxShadow: [
            BoxShadow(
